      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "32px 40px", display: "flex", flexDirection: "column" }}>
            {/* ASCII Art */}
            <div style={{ fontSize: "9px", fontFamily: "var(--font-mono)", color: "rgba(232,234,240,0.12)", lineHeight: "1.2", marginBottom: "20px", textAlign: "center", whiteSpace: "pre", userSelect: "none" }}>
{`  ___ _ _ _      _      _   ___
 | __| | (_)___| |_   /_\\ |_ _|
 | _|| | | / _ \\  _| / _ \\ | |
 |___|_|_|_\\___/\\__|/_/ \\_\\___|`}
            ...
            {/* Welcome Text */}
            <div style={{ color: "var(--text-secondary)", fontSize: "14px", lineHeight: "1.6", marginBottom: "20px", maxWidth: "700px" }}>
              Organization AI Engineer — connected to your repositories, standards, and knowledge base. I plan, build, test and validate with full context of <span style={{ color: "var(--accent-blue)" }}>Payments Platform</span>.
            </div>

            {/* Suggestion Chips */}
            <div style={{ display: "flex", gap: "8px", marginBottom: "20px", flexWrap: "wrap" }}>
              {["create unit tests for the refund service", "/agents", "/context", "/help"].map((chip) => (
                <button key={chip} onClick={() => setInput(chip)} style={{ background: "transparent", border: "1px solid var(--border)", color: "var(--text-secondary)", padding: "5px 12px", borderRadius: "4px", fontSize: "11px", cursor: "pointer", fontFamily: "var(--font-sans)", transition: "all 0.15s" }} onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.borderColor = "var(--accent-blue)"; (e.currentTarget as HTMLButtonElement).style.color = "var(--text-primary)"; }} onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.borderColor = "var(--border)"; (e.currentTarget as HTMLButtonElement).style.color = "var(--text-secondary)"; }}>{chip}</button>
              ))}
            </div>

            {running && (
              <div style={{ padding: "12px", background: "var(--surface-2)", borderRadius: "4px", fontSize: "12px", color: "var(--text-secondary)", marginBottom: "12px" }}>
                <span style={{ display: "inline-block", animation: "spin 0.8s linear infinite", width: "8px", height: "8px", border: "1.5px solid var(--border)", borderTopColor: "var(--accent-blue)", borderRadius: "50%", marginRight: "8px", verticalAlign: "middle" }} /> Elliot is thinking...
              </div>
            )}
          </div>
          ...
          <div style={{ padding: "12px 20px", borderTop: "1px solid var(--border)", background: "var(--surface)", flexShrink: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 12px", background: "var(--surface-2)", borderRadius: "4px", border: "1px solid var(--border)" }}>
              ...
              <input
                ...
                placeholder="Ask Elliot, or type / for commands"
                style={{ flex: 1, background: "transparent", border: "none", outline: "none", color: "var(--text-primary)", fontSize: "13px", fontFamily: "var(--font-mono)" }}
              />
              <span style={{ color: "var(--text-muted)", fontSize: "10px" }}>↵ to run</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: "6px", fontSize: "10px", color: "var(--text-muted)" }}>
              <span style={{ fontFamily: "var(--font-mono)" }}>[commands] [Tab complete] [↑ history]</span>
